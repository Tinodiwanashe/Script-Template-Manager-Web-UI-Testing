...
class Utils {
	...
	/**
	 * Opens the Web Application and maximize the window
	 * @param Url used to redirect to web application.
	 */
	@Keyword
	def openWebApp(String url, int timeout){
		WebUiBuiltInKeywords.openBrowser(url, FailureHandling.STOP_ON_FAILURE)
		WebUiBuiltInKeywords.waitForPageLoad(timeout, FailureHandling.STOP_ON_FAILURE);
		WebUiBuiltInKeywords.maximizeWindow(FailureHandling.STOP_ON_FAILURE)
		KeywordUtil.markPassed("Web Application Opened successfully")
	}
